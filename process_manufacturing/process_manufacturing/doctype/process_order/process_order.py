		...
		for po in doc1:
			...
			if(po.name==se.process_order and se.stock_entry_type == 'Process Manufacturing'):
				...
				for oc in opcost.get("operation_cost"):
					...
					child_row.expense_account = oc.operations
					child_row.amount = oc.cost
					child_row.base_amount = oc.cost
					child_row.description = "None"
					addcost.save()
...
class ProcessOrder(Document):
	...
	def qtyupdate(self):
		...
		self.total_all_amount=(self.finished_products_amount+float(self.total_operation_cost))-self.scrap_amount
